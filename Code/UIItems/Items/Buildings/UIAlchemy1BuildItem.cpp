#include "StdAfx.h"
#include "UIAlchemy1BuildItem.h"
#include "GamePlugin.h"

#include <Components/Player/Player.h>
#include <CryEntitySystem/IEntitySystem.h>
#include <Components/BaseBuilding/BaseBuilding.h>
#include <Components/BaseBuilding/Buildings/Alchemy1Building.h>
...
UIAlchemy1BuildItem::UIAlchemy1BuildItem(IEntity* entity)
{
	this->m_pEntity = entity;
}

void UIAlchemy1BuildItem::Execute()
{
	if (!m_pEntity) {
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "UIAlchemy1BuildItem : (Execute) m_pEntity is null !");
		return;
	}

	IEntity* m_pPlayerEntity = gEnv->pEntitySystem->FindEntityByName(PLAYER_ENTITY_NAME);
	if (!m_pPlayerEntity) {
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "UIAlchemy1BuildItem : (Execute) m_pPlayerEntity is null !");
		return;
	}
	OwnerInfoComponent* pOwnerInfo = m_pEntity->GetComponent<OwnerInfoComponent>();
	if (!pOwnerInfo) {
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "UIAlchemy1BuildItem : (Execute) pOwnerInfo | OwnerEntity is null !");
		return;
	}
	IEntity* ownerEntity = pOwnerInfo->GetOwner();
	if (!ownerEntity) {
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "UIAlchemy1BuildItem : (Execute) ownerEntity is null !");
		return;
	}
	...
	if (!resourceManager->RequsetResources(Alchemy1BuildingComponent::GetCost())) {
		return;
	}
	...
	if (actionManager) {
		...
		if (pBuildingEntity) {
			pBuildingEntity->GetOrCreateComponent<Alchemy1BuildingComponent>();
		}
	}
}
